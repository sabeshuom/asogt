...
def add_time_table(wb, ws, start_time, comps,  stats_ref_range, total_time_col, comp_type = "oral"):
    ...
    data_range_start = "${:s}${:d}".format(alpha_u[top_c], top_r + header_rows +1)
    # ws.conditional_format('A1:A4', {'type':     'formula',
    #                                    'criteria': '=$A$1>5',
    #                                    'format':   comp_duplicate_format})

    data_range_finish ="${:s}${:d}".format(alpha_u[(columns_per_room +1)* room_split+1] , ((len(rooms)/room_split) +1) * (rows_per_room + header_rows + gap_between_rooms) +1)
    for r, comp_item in enumerate(comps.items()):
        ...
        cell_range = "A{:d}:C{:d}".format(r + header_rows + 1, r + header_rows + 1)
    ...
    for room_index, room in enumerate(rooms):
        ...
        start_r = top_r + room_index/room_split * (rows_per_room + header_rows + gap_between_rooms) +1 

        ws.set_column('{:s}:{:s}'.format(alpha_u[start_c], alpha_u[start_c + columns_per_room-1]), 15)
        # set start time
        ...
        room_cell = "{:s}{:d}:{:s}{:d}".format(alpha_u[start_c], start_r+1, alpha_u[start_c + columns_per_room-1], start_r +1)
        ws.merge_range(room_cell, room_name, cell_room_format)
        
        # set time table header
        ws.set_row(start_r+1, 25) # row index starts 0
        room_cell = "{:s}{:d}:{:s}{:d}".format(alpha_u[start_c], start_r+1, alpha_u[start_c + columns_per_room-1], start_r +1)
        ws.write_row(start_r+1, start_c, ["Comp", "Start Time", "End Time"], cell_room_format)
        ...
        for count in range(rows_per_room):
            ...
            if (comp_type == "oral") and (count > 0):
                st = '=IF(ISBLANK({:s}), "", {:s})'.format(comp_cell, "{:s}{:d}".format(alpha_u[start_c+2], current_row -1))
            else:
                st = '=IF(ISBLANK({:s}), "", {:s})'.format(comp_cell, start_time_cell)

            et = '=IF(ISBLANK({:s}),"", MROUND({:s} + TIME(0, VLOOKUP({:s},{:s},{:d},FALSE),0),"0:15"))'.format( comp_cell, st_cell,comp_cell, stats_ref_range, total_time_col)
...
if __name__ == "__main__":
    ...
    add_time_table(wb, ws,  start_time, comps,  stats_ref_range, total_time_col, comp_type="oral")
    ws = wb.add_worksheet("Time Table 05-08-2018")
    add_time_table(wb, ws,  start_time, comps, stats_ref_range, total_time_col, comp_type="written")
    wb.close()